...
Задача модели — закодировать контекст предложения в «полезный» вектор, который далее сравнивается с эмбеддингом целевого слова для замены с помощью скалярного произведения. Если результат сравнения $\in[0, 1]$ больше некоторого порогового значения, производим замену.
...
Важные моменты в реализации предобработки:

- Эмбеддинги слов инициализируются предобученными векторами из Navec, слова с буквой Ё из исходного словаря игнорируются, однако для слов, у которых доступны варианты с Е и Ё одновременно, от них взято среднее. Например, для слова «все» изначальный эмбеддинг будет суммой эмбеддингов «все» и «всё». Это нужно для того, чтобы не потерять важную разницу, которую уже успели выучить вектора и не тратить капасити нашей модели на это.
...
Обучение происходило на RXT 3070 8Gb в среднем менее чем за 10 минут.

В ходе обучения использовался достаточно тривиальный график `learning_rate` — уменьшение при попадании на плато.

## Метрики

Метрики на тестовой выборке доступны в файле [metrics.json](metrics.json), на обучении доступны в директории [dvclive](../dvclive/) или с помощью вызова команды (из корня проекта):
...
- Эксперименты с другим графиком `learning_rate`
