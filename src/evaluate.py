...
class EvaluateResult:
    ...
    def __str__(self):
        return "\n".join(["{}: {}".format(m, v) for m, v in self.__dict__.items()])


def get_substrings(text: str) -> List[List[int]]:
    """Function for get ye(yo) words indexes"""
    res = []
...
def substrings2onehot(text:str, positions: List[Tuple[int, int]]) -> np.ndarray[bool]:
    """Transform substring positions to one hot vector"""
